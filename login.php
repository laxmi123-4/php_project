...
<head>
    ...
    <title>Hostel Admission - Login</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="bg-white p-8 rounded-lg shadow-md w-96">
        <h2 class="text-2xl font-bold text-center text-blue-700 mb-4">User Login</h2>

        <?php if ($error): ?>
            <p class="text-red-500 text-sm text-center mb-4"><?php echo htmlspecialchars($error); ?></p>
        <?php endif; ?>
        <?php if ($success): ?>
            <p class="text-green-500 text-sm text-center mb-4"><?php echo htmlspecialchars($success); ?></p>
        <?php endif; ?>

        <form action="auth_controllerL.php" method="post" class="space-y-4">
